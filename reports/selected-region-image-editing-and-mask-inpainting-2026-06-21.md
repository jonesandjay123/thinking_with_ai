...
日期：2026-06-21  
主題：哪些現成工具、模型與 API 能做到「只修改合成圖中間 X 區域，盡量不動左右 A/B 區域」？

## TL;DR

一句話結論：這件事不是新問題，成熟方向叫做 selected-region editing / mask-based inpainting / generative fill；但「模型支援 mask」不等於「輸出保證 mask 外 pixel-perfect 不變」。如果我們的工程需求是 `[A][X][B]` 之後還要接回原 pipeline，最穩的做法是兩段式：
...
也就是：用 inpainting model 生成 X，但最後用程式把 A/B 從原圖硬貼回去。這才是 pixel-preserve outside mask 的工程保險。
...
2. **Adobe Firefly API / Photoshop Generative Fill 作為人工和商業可用 baseline**  
   Adobe 的 GUI 和 API 都明確支援 selection / mask / generative fill，適合快速驗證「人類修圖效果上限」。但 Photoshop GUI 不適合全自動 pipeline；Firefly API 需要 Adobe 開發者流程。
...
- 理想狀態是 mask 外 pixel-preserve；至少要可檢測、可修正、可 deterministic composite。
...
它適合人工修圖，但不一定等於工程上可重複、可驗證、可 pixel-perfect。
...
模型層級的「preserve」通常表示模型會嘗試不修改或少修改 mask 外內容，但不一定保證 byte-for-byte / pixel-for-pixel 一樣。原因包括：
...
3. 用 hard mask / alpha composite 把 X 貼回原圖。
4. 對 A/B 區域跑 pixel diff，確認完全一致。
...
- pixel preserve：PSD 裡原圖 layer 可保留；但最終匯出還是應對 A/B 跑 diff。
...
- 但仍要實測輸出是否 byte-identical；即使文件說 black protected，也建議 final hard composite。
...
- 自動：Firefly API 值得試。
- pixel preserve：文件語義強，但仍要驗證；工程上仍建議 hard composite。
...
- 但模型較重，API 或本地配置成本較高。
- 即使官方寫 black preserved，工程上仍建議 final hard composite。
