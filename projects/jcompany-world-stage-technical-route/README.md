# Jcompany World Stage 技術路線調查報告

> 撰寫時間：2026-06-12  
> 對象：Jcompany 固定鏡頭 / 導演式鏡頭 cinematic web world  
> 核心邊界：Jcompany 是祈願絲帶世界，不是遊戲；沒有角色、沒有自由移動、沒有自由旋轉鏡頭，不做 full open world。

## 0. Context

Jcompany 想做的是一個在 web 上運行、能和 React / HTML UI / wish flow 整合的「祈願絲帶世界舞台」。它需要讓使用者感覺自己看見一個高質感、會呼吸的世界，但技術目標不是重建一套完整 3A engine。
...
我不建議 Jcompany 第一階段走純 B1，也不建議走純 C。

純 B1 的問題是：agent 很容易做出能跑的 Three.js scene，但若背景、光影、美術方向、材質、霧、鏡頭節奏沒有被強力約束，成品會快速滑向「便宜 3D demo」。R3F 很適合實作互動層，但不應承擔整個高質感世界的美術來源。

純 C 的問題是：Blender / Unity / Unreal 很適合做漂亮場景和鏡頭，但若最後變成整個 Unity / Unreal WebGL runtime 嵌進網站，會太重、web UI 整合差、迭代慢，也和 Jcompany 的「祈願流程 / web product」本質不合。
...
B1 很可行，而且是 Jcompany web runtime 的核心。Three.js 是 browser 3D 的成熟基礎；R3F 是 Three.js 的 React renderer，適合把 3D scene 做成 React component，和網站 state、routing、HTML overlay 整合。R3F 官方 performance 文件也明確提醒：不要在 `useFrame` 或 fast events 裡反覆 `setState`，應直接 mutate 物件或用 delta / spring 類方式處理連續動畫。這對 Jcompany 很重要，因為 ribbon、particles、wind 都是 per-frame 動畫。
...
- [R3F performance pitfalls](https://r3f.docs.pmnd.rs/advanced/pitfalls)：必讀。Jcompany 的風、ribbon、particles 不應每幀觸發 React state 更新。
...
- [Drei showcase / helpers](https://storybook.js.org/showcase/pmndrs-drei/)：Drei 是 R3F 常用 helper 集合；Jcompany 可用 `Html`、`Environment`、`Billboard`、`Float`、`Cloud` 類工具，但要小心 helper demo 味。
...
- [GSAP ScrollTrigger docs](https://gsap.com/docs/v3/Plugins/ScrollTrigger/)：適合 scroll-linked 或 staged entrance；但 Jcompany prototype 先用 click-to-focus，不必一開始做 heavy scroll cinematic。
...
- [Codrops depth-map / WebGPU scanning effect](https://tympanus.net/codrops/2025/03/31/webgpu-scanning-effect-with-depth-maps/)：值得參考「深度圖 + shader 讓 2D image 有空間感」；Jcompany 不應照抄 scanning / sci-fi 效果。
...
- Jcompany 沒有角色控制和大型互動，不值得背 Unity runtime 成本
...
Unreal Sequencer / Movie Render Pipeline 是做 cinematic render 的強項。若 Jcompany 要一張非常漂亮的「世界舞台主視覺」或 trailer-like background，Unreal 很有吸引力。

但 Unreal 不適合：

- 作為 Jcompany web runtime
...
- Unity 的強項偏互動遊戲 / camera tool / asset store；Jcompany 第一階段不需要 Unity runtime。
...
- Mixamo：基本無關。它主打 3D characters、rigging、character animations；Jcompany 沒有人物角色，所以除非未來做「布條骨架動畫」旁路，不應成為主要工具。
...
Jcompany 第一版應該選中間：預渲染遠景 + R3F 局部即時互動。

## 5. Route D — Hybrid Cinematic Web Diorama

### 這是否是最適合 Jcompany 的 production pipeline

是。Route D 最符合 Jcompany 的真實需求。

Jcompany 不需要做「可探索的 3D 世界」。它需要的是：
...
> **Jcompany World Stage P0 — One Anchor, One Ribbon, One Wish**
...
6. 自製 ribbon texture：不要用隨機 cloth texture；要符合 Jcompany 色彩和厚薄。
...
適合。Three.js 是 Jcompany runtime 的底層 3D library。它適合 scene、camera、mesh、shader、particles、GLB props、raycasting。若未來需要更細 shader 控制，可以直接用 Three.js primitives。

注意：直接 raw Three.js 和 React app 整合時，要自己管理 lifecycle、resize、dispose、state bridge。

#### React Three Fiber / R3F

非常適合。Jcompany 是 React / HTML UI + 3D world 的混合產品，R3F 的 component model 比 raw Three.js 更好維護。它也適合 AI agent 生成結構化 component。
...
適合 camera / object staged timeline，尤其 click-to-focus、ritual sequence、UI entrance。ScrollTrigger 也成熟，但 Jcompany 第一版不必過度 scroll-driven。
...
現在的實際 production target。Three.js / R3F 的主要穩定 runtime 仍是 WebGL。Jcompany 第一版應以 WebGL + graceful fallback 為主。
...
可作為 2D layered diorama alternative，但不是主推薦。PixiJS 是高效 2D WebGL / WebGPU renderer，很適合 2D particles、parallax、sprite layers。如果 Jcompany 完全不想碰 3D props，PixiJS 很合理。

但 Jcompany 需要少量 3D ribbon / anchors / camera depth illusion，R3F 更合適。PixiJS 可作為 pure 2D fallback 或 loading/ritual animation layer。
...
建議：可用 Spline 做 mood prototype，不作為 Jcompany world stage 的主架構。

#### Theatre.js

很適合 Jcompany 的「導演式鏡頭」。Theatre.js 官方提供 Three.js / R3F integration，可用視覺化 keyframe 控制 camera、lights、material colors、object transforms。
...
Jcompany 的 ribbon 是核心，必須特別小心。最重要原則：
...
不推薦作為 Jcompany ribbon 主體。`TubeGeometry` 很容易變成 rope、hose、snake、plastic tube。除非要做繩子、桿子、光線軌跡，否則不要用來代表布條。
...
最重要的是：這套不會把 Jcompany 綁死在 Unity/Unreal WebGL，也不會讓純 Three.js 從零承擔所有美術。
...
- 它是可駕駛 / 可探索 / game-like portfolio，和 Jcompany 的固定鏡頭路線相反。
- Jcompany 不需要車、不需要物理、不需要開放地圖。
...
- 很多 Awwwards site 追求 novelty / shock / scroll spectacle，Jcompany 需要安靜、祈願、儀式感。
...
- demo visual style。Jcompany 需要自有 art direction。
...
- scanning light / sci-fi shader 不是 Jcompany 氣質。
...
- Jcompany 第一版不需要 gallery 或強 scroll mechanic。
...
- 範例偏抽象科技感；Jcompany ribbon 要更柔、更薄、更有布感和祈願感。
...
- 6-12 張 Jcompany world stage mood frames。
- 選 1 張作 P0 背景 plate。
