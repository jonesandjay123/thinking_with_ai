# Jovicheer 水平無縫循環背景研究

> 2026-06-08 研究整理。目標是幫 Jovicheer 第一版首頁做一個穩定、輕量、溫柔的水平環繞背景：柔和霧氣、奶油金 / 淡粉 / 柔白光源、空氣深度、漂浮 wish ribbons，使用者像站在原地慢慢左右觀看一個會回到原位的空間。

## 結論先講

Jovicheer 第一版不建議繼續把「天空色塊、太陽、霧、ribbon placeholders」全部放在一個 4x viewport world 裡一起移動，然後靠複製三份來補 buffer。這種做法只要左邊界和右邊界不是同一張可平鋪材質的自然延續，接縫一定會出現；霧氣和柔光尤其容易露餡，因為人眼對大面積低頻漸層的不連續很敏感。
...
- 視覺最貼近 Jovicheer：柔霧、光、空氣感都能靠美術資產和 CSS blend 完成。
...
Jovicheer 第一版不需要真正 3D 霧體。大量 particles、raymarching volumetric fog、複雜 postprocessing 都會讓手機和普通筆電變成風險點，而且視覺容易偏科幻遊戲。
...
1. 先生成一張符合 Jovicheer 氛圍的寬圖，但 prompt 避免具象雲朵和強物件：
...
5. 若出現 AI 新增具象物件，刪掉重修；Jovicheer 背景應該讓 ribbon 是唯一明顯語彙。
...
```text
請重構 Jovicheer 首頁背景的水平循環架構，目標是消除霧氣 / 光源 / 背景色塊 seam，保持手機和普通筆電穩定。
...
Jovicheer 的背景不是要炫技，而是要讓願望、ribbon、祈願感站在前面。第一版應該把工程風險壓低：固定底光 + tileable fog + ribbon wrap。等視覺和互動方向穩了，再考慮 Canvas 或 Three.js 做更自然的霧氣微動。
